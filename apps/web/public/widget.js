"use strict";(()=>{var oe=Object.defineProperty;var se=(i,e,t)=>e in i?oe(i,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):i[e]=t;var f=(i,e,t)=>se(i,typeof e!="symbol"?e+"":e,t);function re(i){return i.endsWith("/")?i.slice(0,-1):i}var I=class{constructor(e){f(this,"baseUrl");this.baseUrl=re(e)}async getConfig(e){let t=await fetch(`${this.baseUrl}/widget/config/${encodeURIComponent(e)}`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"omit"});if(!t.ok)throw new Error(`WIDGET_CONFIG_FAILED:${t.status}`);return await t.json()}async getConsultationSnapshot(e,t){let n=await fetch(`${this.baseUrl}/widget/consultation/${encodeURIComponent(e)}/${encodeURIComponent(t)}`,{method:"GET",headers:{"Content-Type":"application/json"},credentials:"omit"});if(!n.ok)throw new Error(`WIDGET_SNAPSHOT_FAILED:${n.status}`);return await n.json()}async sendChat(e,t="/chat/messages"){let n=t.startsWith("/")?t:`/${t}`,s=await fetch(`${this.baseUrl}${n}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e),credentials:"omit"});if(!s.ok)throw new Error(`WIDGET_CHAT_FAILED:${s.status}`);return await s.json()}async sendFeedback(e,t){await fetch(`${this.baseUrl}/chat/messages/${encodeURIComponent(e)}/feedback`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({feedback:t}),credentials:"omit"})}async streamChat(e,t,n="/chat/messages/stream"){let s=n.startsWith("/")?n:`/${n}`,o=await fetch(`${this.baseUrl}${s}`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify(e),credentials:"omit"});if(!o.ok||!o.body)throw new Error(`WIDGET_CHAT_STREAM_FAILED:${o.status}`);let r=o.body.getReader(),a=new TextDecoder("utf-8"),d="";for(;;){let{value:c,done:u}=await r.read();if(u)break;d+=a.decode(c,{stream:!0});let p=d.indexOf(`

`);for(;p!==-1;){let h=d.slice(0,p).trim();if(d=d.slice(p+2),p=d.indexOf(`

`),!h)continue;let g="message",b=[];for(let m of h.split(`
`))m.startsWith("event:")?g=m.slice(6).trim():m.startsWith("data:")&&b.push(m.slice(5).trim());if(b.length!==0)try{let m=JSON.parse(b.join(`
`));t({event:g,data:m})}catch{t({event:"error",data:{code:"STREAM_EVENT_PARSE_FAILED",message:"\uC2A4\uD2B8\uB9BC \uC774\uBCA4\uD2B8 \uD30C\uC2F1 \uC2E4\uD328"}})}}}}};function ae(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function k(i){let e=i;return e=e.replace(/`([^`]+)`/g,(t,n)=>`<code>${n}</code>`),e=e.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g,(t,n,s)=>`<a href="${s}">${n}</a>`),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/(^|[^*])\*([^*\n]+)\*(?!\*)/g,"$1<em>$2</em>"),e=e.replace(/(^|[^_])_([^_\n]+)_(?!_)/g,"$1<em>$2</em>"),e}function D(i){let e=i.trim();return e.startsWith("|")&&(e=e.slice(1)),e.endsWith("|")&&(e=e.slice(0,-1)),e.split("|").map(t=>t.trim())}var _=/^\s*\|.*\|\s*$/,le=/^\s*\|?[\s:-]*-[-\s:|]*\|?\s*$/,N=/^(#{1,6})\s+(.*)$/,U=/^\s*[-*]\s+(.*)$/,O=/^\s*\d+\.\s+(.*)$/,P=/^\s*>\s+(.*)$/;function Q(i){return/(^|\n)\s*\|.*\|/.test(i)||/\*\*[^*]+\*\*/.test(i)||/(^|\n)\s*[-*]\s+\S/.test(i)||/(^|\n)\s*\d+\.\s+\S/.test(i)||/(^|\n)#{1,6}\s+\S/.test(i)||/\[[^\]]+\]\([^)]+\)/.test(i)||/(^|\n)>\s+\S/.test(i)}function j(i){let e=ae(i.replace(/\r\n/g,`
`)).split(`
`),t=[],n=null,s=()=>{n&&(t.push(`</${n}>`),n=null)},o=0;for(;o<e.length;){let r=e[o];if(_.test(r)&&o+1<e.length&&le.test(e[o+1])){s();let h=D(r);o+=2;let g=[];for(;o<e.length&&_.test(e[o]);)g.push(D(e[o])),o+=1;let b="<table><thead><tr>"+h.map(m=>`<th>${k(m)}</th>`).join("")+"</tr></thead><tbody>";for(let m of g)b+="<tr>"+m.map(x=>`<td>${k(x)}</td>`).join("")+"</tr>";b+="</tbody></table>",t.push(b);continue}let a=r.match(N);if(a){s(),t.push(`<h${a[1].length}>${k(a[2])}</h${a[1].length}>`),o+=1;continue}let d=r.match(U);if(d){n!=="ul"&&(s(),t.push("<ul>"),n="ul"),t.push(`<li>${k(d[1])}</li>`),o+=1;continue}let c=r.match(O);if(c){n!=="ol"&&(s(),t.push("<ol>"),n="ol"),t.push(`<li>${k(c[1])}</li>`),o+=1;continue}let u=r.match(P);if(u){s(),t.push(`<blockquote>${k(u[1])}</blockquote>`),o+=1;continue}if(r.trim()===""){s(),o+=1;continue}s();let p=[r];for(o+=1;o<e.length&&e[o].trim()!==""&&!_.test(e[o])&&!N.test(e[o])&&!U.test(e[o])&&!O.test(e[o])&&!P.test(e[o]);)p.push(e[o]),o+=1;t.push(`<p>${p.map(k).join("<br>")}</p>`)}return s(),t.join("")}var de=new Set(["p","br","div","span","section","article","header","footer","small","sub","sup","h1","h2","h3","h4","h5","h6","strong","em","b","i","u","s","mark","code","pre","ul","ol","li","a","table","thead","tbody","tr","td","th","caption","colgroup","col","blockquote","hr","figure","figcaption","img"]),ce={a:new Set(["href","title"]),img:new Set(["src","alt","title","width","height"]),td:new Set(["colspan","rowspan"]),th:new Set(["colspan","rowspan","scope"]),col:new Set(["span"]),colgroup:new Set(["span"])},ue=new Set(["color","background","background-color","margin","margin-top","margin-right","margin-bottom","margin-left","padding","padding-top","padding-right","padding-bottom","padding-left","gap","row-gap","column-gap","border","border-top","border-right","border-bottom","border-left","border-width","border-style","border-color","border-radius","border-collapse","border-spacing","font","font-size","font-weight","font-style","font-family","line-height","letter-spacing","text-align","text-decoration","text-transform","white-space","word-break","overflow-wrap","vertical-align","width","min-width","max-width","height","min-height","max-height","box-sizing","display","flex","flex-direction","flex-wrap","flex-grow","flex-shrink","flex-basis","align-items","align-self","justify-content","justify-items","grid-template-columns","grid-template-rows","grid-gap","list-style","list-style-type","list-style-position","box-shadow","opacity","overflow","overflow-x","overflow-y"]);function pe(i){let e=[];for(let t of i.split(";")){let n=t.indexOf(":");if(n<0)continue;let s=t.slice(0,n).trim().toLowerCase(),o=t.slice(n+1).trim();if(!ue.has(s))continue;let r=o.toLowerCase();r.includes("url(")||r.includes("expression")||r.includes("javascript:")||r.includes("@import")||r.includes("</")||r.includes("fixed")||r.includes("absolute")||r.includes("sticky")||e.push(`${s}: ${o}`)}return e.join("; ")}var he=/<[a-zA-Z][a-zA-Z0-9]*(\s|>|\/)/;function F(i){return he.test(i)}function q(i){let e=i.trim().toLowerCase();return!(!e||e.startsWith("javascript:")||e.startsWith("data:")||e.startsWith("vbscript:")||e.startsWith("file:"))}function G(i,e){let t=Array.from(i.children);for(let n of t){let s=n.tagName.toLowerCase();if(!de.has(s)){let r=e.createTextNode(n.textContent||"");n.replaceWith(r);continue}let o=ce[s]||new Set;for(let r of Array.from(n.attributes)){let a=r.name.toLowerCase();if(a==="style"){let d=pe(r.value);d?n.setAttribute("style",d):n.removeAttribute("style");continue}o.has(a)||n.removeAttribute(r.name)}if(s==="a"){let r=n.getAttribute("href")||"";q(r)||n.removeAttribute("href"),n.setAttribute("target","_blank"),n.setAttribute("rel","noopener noreferrer")}if(s==="img"){let r=n.getAttribute("src")||"";q(r)||n.removeAttribute("src")}G(n,e)}}function $(i){if(typeof window>"u"||typeof DOMParser>"u")return i.replace(/<[^>]*>/g,"");if(!i)return"";let t=new DOMParser().parseFromString(`<div id="__ieum_root__">${i}</div>`,"text/html"),n=t.getElementById("__ieum_root__");return n?(G(n,t),n.innerHTML):""}var me="/widget-icons/love-chat-icons.png",fe=8,ge=6,be=4,K="\uC694\uCCAD \uCC98\uB9AC \uC911 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4. \uC7A0\uC2DC \uD6C4 \uB2E4\uC2DC \uC2DC\uB3C4\uD574 \uC8FC\uC138\uC694.",xe="\uAC1C\uC778\uC815\uBCF4\uAC00 \uD3EC\uD568\uB41C \uB0B4\uC6A9\uC740 \uC785\uB825\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4. \uAC1C\uC778\uC815\uBCF4\uB97C \uC81C\uC678\uD558\uACE0 \uB2E4\uC2DC \uC785\uB825\uD574 \uC8FC\uC138\uC694.",V="AI \uC774\uC74C\uBD07\uB3C4 \uAC00\uB054 \uC2E4\uC218\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4. \uC911\uC694\uD55C \uC815\uBCF4\uB294 \uAF2D \uB2E4\uC2DC \uD55C\uBC88 \uD655\uC778\uD558\uC138\uC694.",ve=[/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/,/\b\d{6}-[1-4]\d{6}\b/,/\b\d{4}[- ]?\d{4}[- ]?\d{4}[- ]?\d{4}\b/,/\b01[016789][- ]?\d{3,4}[- ]?\d{4}\b/,/\b(?:19|20)\d{2}[-./](?:0[1-9]|1[0-2])[-./](?:0[1-9]|[12]\d|3[01])\b/];function we(i){return i.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Y(i){return`<img class="ieum-launcher-image" src="${we(i.trim())}" alt="" aria-hidden="true" />`}function l(i,e,t){let n=i.createElement(e);return t&&(n.className=t),n}function M(i,e){let t=e||"";F(t)?(i.classList.add("ieum-bubble-rich"),i.innerHTML=$(t)):Q(t)?(i.classList.add("ieum-bubble-rich"),i.innerHTML=$(j(t))):(i.classList.remove("ieum-bubble-rich"),i.textContent=t)}function y(i,e){return i==="custom"&&e?.trim()?Y(e):i==="love-chat"?Y(me):i==="heart"?`
  ...
  `}function ye(i){return i&&i.trim()?i.replace(/\/$/,""):`${window.location.origin}/api`}function L(i){return typeof i=="string"?i:void 0}function ke(i){return typeof i=="boolean"?i:void 0}function Ce(i){return Array.isArray(i)?i:[]}function Le(i){return Array.isArray(i)?i:[]}var v='fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"',te={doc:`<svg viewBox="0 0 24 24" ${v}><path d="M6 3h8l4 4v14H6z"/><path d="M14 3v4h4"/><path d="M9 12h6M9 16h6"/></svg>`,shield:`<svg viewBox="0 0 24 24" ${v}><path d="M12 3l7 3v5c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6z"/><path d="M9 12l2 2 4-4"/></svg>`,member:`<svg viewBox="0 0 24 24" ${v}><circle cx="10" cy="8" r="3.2"/><path d="M4 20c0-3.3 2.7-5.6 6-5.6 1.2 0 2.3.3 3.2.8"/><path d="M18 14v6M15 17h6"/></svg>`,cert:`<svg viewBox="0 0 24 24" ${v}><circle cx="12" cy="9" r="5.2"/><path d="M9.7 9l1.6 1.6 3-3.2"/><path d="M8.5 13.2 7 20l5-2.6L17 20l-1.5-6.8"/></svg>`,search:`<svg viewBox="0 0 24 24" ${v}><circle cx="11" cy="11" r="6"/><path d="M20 20l-4.3-4.3"/></svg>`,phone:`<svg viewBox="0 0 24 24" ${v}><path d="M6.6 10.8a12 12 0 0 0 5.6 5.6l1.9-1.9a1 1 0 0 1 1-.24 11 11 0 0 0 3.4.55 1 1 0 0 1 1 1V20a1 1 0 0 1-1 1A16 16 0 0 1 3 5a1 1 0 0 1 1-1h3.3a1 1 0 0 1 1 1 11 11 0 0 0 .55 3.4 1 1 0 0 1-.24 1z"/></svg>`,apply:`<svg viewBox="0 0 24 24" ${v}><path d="M5 4h9l4 4v6"/><path d="M14 4v4h4"/><path d="M13 21l-4 1 1-4 6.5-6.5a1.4 1.4 0 0 1 2 2z"/></svg>`,check:`<svg viewBox="0 0 24 24" ${v}><circle cx="12" cy="12" r="8.5"/><path d="M8.5 12.5l2.5 2.5 4.5-5"/></svg>`,info:`<svg viewBox="0 0 24 24" ${v}><circle cx="12" cy="12" r="8.5"/><path d="M12 11v5"/><path d="M12 8h.01"/></svg>`,won:`<svg viewBox="0 0 24 24" ${v}><circle cx="12" cy="12" r="8.5"/><path d="M8 9l1.6 6L12 10l2.4 5L16 9"/><path d="M7.4 11.5h9.2"/></svg>`,grid:`<svg viewBox="0 0 24 24" ${v}><rect x="4" y="4" width="7" height="7" rx="1.5"/><rect x="13" y="4" width="7" height="7" rx="1.5"/><rect x="4" y="13" width="7" height="7" rx="1.5"/><rect x="13" y="13" width="7" height="7" rx="1.5"/></svg>`,chat:`<svg viewBox="0 0 24 24" ${v}><path d="M4 5h16v11H9l-4 3v-3H4z"/><path d="M8 9h8M8 12h5"/></svg>`,calendar:`<svg viewBox="0 0 24 24" ${v}><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M4 9h16M8 3v4M16 3v4"/></svg>`,building:`<svg viewBox="0 0 24 24" ${v}><path d="M5 21V5a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v16"/><path d="M15 9h3a1 1 0 0 1 1 1v11"/><path d="M8 8h4M8 12h4M8 16h4"/></svg>`},Te=/^(https?:\/\/|tel:|mailto:|\/)/i;function Me(i){let e=i.trim(),t="none",n="",s=e,o=e.match(/^\[([a-z0-9_-]+)\]\s*([\s\S]*)$/i);if(o&&te[o[1].toLowerCase()])t="svg",n=o[1].toLowerCase(),s=o[2].trim();else{let u=e.search(/\s/);if(u>0){let p=e.slice(0,u);!/[0-9A-Za-z가-힣]/.test(p)&&/[←-⯿️‍]|[\u{1F000}-\u{1FAFF}]/u.test(p)&&(t="emoji",n=p,s=e.slice(u+1).trim())}}let r="",a=s.lastIndexOf(" | ");if(a>0){let u=s.slice(a+3).trim();Te.test(u)&&(r=u,s=s.slice(0,a).trim())}let d="",c=s.indexOf(" :: ");return c>0&&(d=s.slice(c+4).trim(),s=s.slice(0,c).trim()),s||(s=e),{iconType:t,icon:n,label:s,description:d,link:r,raw:e}}function Ee(i){return Array.isArray(i)?i.filter(e=>typeof e=="string"):[]}function Ae(i){return ve.some(e=>e.test(i))}function ie(i){return/^https?:\/\//i.test(i)||/^[\w.-]+\.[a-z]{2,}(?:\/|\?|$)/i.test(i)}function Ie(i){if(!i?.trim())return null;try{return new URL(i.trim()).hostname.replace(/^www\./,"")}catch{return null}}function We(i){return!i||i==="answered"?null:i==="insufficient_evidence"?"\uB4F1\uB85D\uB41C \uC790\uB8CC\uC5D0\uC11C \uAD00\uB828 \uC815\uBCF4\uB97C \uCDA9\uBD84\uD788 \uCC3E\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.":i==="restricted"?"\uC548\uC804\uD55C \uC548\uB0B4 \uBC94\uC704\uC5D0\uC11C \uB2F5\uBCC0\uC774 \uC81C\uD55C\uB41C \uC9C8\uBB38\uC785\uB2C8\uB2E4.":i==="conflict"?"\uADFC\uAC70 \uD655\uC778\uC774 \uB354 \uD544\uC694\uD55C \uC9C8\uBB38\uC785\uB2C8\uB2E4.":i==="escalate"?"\uC815\uD655\uD55C \uD655\uC778\uC774 \uD544\uC694\uD55C \uB0B4\uC6A9\uC785\uB2C8\uB2E4.":null}function _e(i,e){let t=e?.trim()||null,n=i.documentName?.trim()||"\uCD9C\uCC98",s=i.pageNumber?`p.${i.pageNumber}`:null,o=i.sectionTitle?.trim()||null,r=t&&t!==n?[t,n]:[n];return s&&r.push(s),o&&o!==n&&!ie(o)&&r.push(o),r.join(" | ")}function S(i){let e=i.sectionTitle?.trim();if(e&&!ie(e))return e;let t=i.documentName?.trim();if(t)return t;let n=i.sourceTitle?.trim();return n||(Ie(i.sourceUrl)??"\uCC38\uC870 \uC790\uB8CC")}function $e(i){return i.some(e=>e.sourceUrl?.trim())?"\uCC38\uC870 \uB9C1\uD06C":"\uCC38\uC870 \uC790\uB8CC"}function Se(i){return i?.citationPresentation==="folded"||i?.citationMode==="compact"}function H(i,e){return e.title?.trim()||i?.institutionName?.trim()||i?.chatbotName?.trim()||"\uAE30\uAD00"}function Z(i,e){let t=e.title?.trim()||i?.chatbotName?.trim()||H(i,e);return t.startsWith("AI \uCC57\uBD07")?t:`AI \uCC57\uBD07 ${t}`}function He(i,e){if(e.welcomeMessage?.trim())return e.welcomeMessage.trim();if(i?.introMessage?.trim())return i.introMessage.trim();if(i?.welcomeMessage?.trim())return i.welcomeMessage.trim();let t=H(i,e);return t&&t!=="\uAE30\uAD00"?`\uC548\uB155\uD558\uC138\uC694. ${t} AI \uC0C1\uB2F4\uBD07\uC785\uB2C8\uB2E4. \uAD81\uAE08\uD558\uC2E0 \uB0B4\uC6A9\uC744 \uD3B8\uD558\uAC8C \uC785\uB825\uD574\uC8FC\uC138\uC694.`:"\uC548\uB155\uD558\uC138\uC694. \uAD81\uAE08\uD558\uC2E0 \uB0B4\uC6A9\uC744 \uC785\uB825\uD574\uC8FC\uC2DC\uBA74 \uBE60\uB974\uAC8C \uC548\uB0B4\uD574\uB4DC\uB9AC\uACA0\uC2B5\uB2C8\uB2E4."}function J(i){return i==="forest"?"linear-gradient(135deg, #166534, #0f766e)":i==="sky"?"linear-gradient(135deg, #1d4ed8, #0284c7)":i==="civic"?"linear-gradient(135deg, #1e40af, #0f766e)":i==="sunset"?"linear-gradient(135deg, #b45309, #ea580c)":"linear-gradient(135deg, #2563EB, #22C55E)"}function Be(i){let e=i?.theme?.launcherIcon;return e==="custom"&&i?.theme?.launcherIconUrl?.trim()?"custom":e==="love-chat"||e==="heart"||e==="shield"||e==="leaf"||e==="spark"?e:"chat"}function Re(i){let e=i.initialLauncherIcon?.trim(),t=i.initialLauncherIconUrl?.trim();return e==="custom"&&t?"custom":e==="love-chat"||e==="heart"||e==="shield"||e==="leaf"||e==="spark"?e:"chat"}function X(i,e){return i==="love-chat"||i==="custom"&&!!e?.trim()}function ze(i,e){let t=i?.launcherHoverMessage?.trim();return t||`${H(i,e)} AI \uC0C1\uB2F4\uBD07\uC785\uB2C8\uB2E4. \uBB34\uC5C7\uC744 \uB3C4\uC640\uB4DC\uB9B4\uAE4C\uC694?`}function De(i){let e=i.match(/#[0-9a-fA-F]{6,8}|#[0-9a-fA-F]{3}/);return e?e[0]:"#2563eb"}function T(i,e){let t=i.replace("#",""),n=t.length===3?t.split("").map(a=>a+a).join(""):t,s=parseInt(n.slice(0,2),16),o=parseInt(n.slice(2,4),16),r=parseInt(n.slice(4,6),16);return`rgba(${s},${o},${r},${e})`}function ee(i){let e=De(i),t=T(e,.18),n=T(e,.35),s=T(e,.4),o=T(e,.28),r=T(e,.12),a=T(e,.08);return`
:host { all: initial; }
.ieum-root, .ieum-root * {
